...
function get_page() {
    ...
    // Remove the trailing slash from the request URI
    $request_uri = rtrim($request_uri, '/');

    // Extract the page name from the request URI
    $page = trim($request_uri, '/');

    // If the page is empty, use the default page based on user type
    if (empty($page)) {
    ...
    // Check if the page exists in the pages folder
    if (file_exists(ROOT_DIR . '/pages/' . $page . '.php')) {
        return $page;
    }

    // Split the page into subfolder and filename
    ...
    // Check if the page exists in the subfolder
